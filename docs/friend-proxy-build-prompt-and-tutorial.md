...
【环境变量】
Secret：
  SERVICE_ACCESS_KEY         — 统一鉴权密钥（所有接口使用）
...
所有 /v1/* 接口均要求鉴权，支持三种方式：
  1. Authorization: Bearer <SERVICE_ACCESS_KEY>   （推荐）
  2. x-goog-api-key: <SERVICE_ACCESS_KEY>          （兼容 Gemini 格式客户端）
  3. ?key=<SERVICE_ACCESS_KEY>                     （URL 查询参数）
...
展示内容：
  ...
  - 一行说明文字："Use this Base URL and your SERVICE_ACCESS_KEY to connect."
...
| 事项 | 说明 |
|---|---|
| **密钥必须一致** | 主代理和所有朋友节点必须使用完全相同的 `SERVICE_ACCESS_KEY` |
